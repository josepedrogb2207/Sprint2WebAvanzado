...
public class KafkaAdminConfig {
    ...
    public KafkaAdmin.NewTopics topics(){
        return new KafkaAdmin.NewTopics(
            ...
            TopicBuilder.name("paciente-topic").partitions(3).replicas(1).build(),
            TopicBuilder.name("citas-topic").partitions(3).replicas(1).build(),
            TopicBuilder.name("estado-topic").partitions(2).replicas(1).build()
        );
    }
}
